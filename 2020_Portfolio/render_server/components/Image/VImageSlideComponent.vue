<template>
    <div>
        <div class="no-image-container" v-if="images.length === 0">
            <div class="no-image">
                NO IMAGE
            </div>
        </div>
        <div v-else class="image-slide-image-container" v-swiper:mySwiper="swiperOption">
            <div class="swiper-wrapper" style="width: 100%" @mouseover="navigationShow" @mouseout="navigationShow">
                <div class="swiper-slide"  v-for="(image, index) in images" :key="index">
                    <v-img class="image-slide-image" :src="`http://localhost:7001/${image.imageUrl}`" alt="" width="100%;"/>
                </div>
            </div>
